...
window.render_gallery = function(user) {
    ...
    container.innerHTML = `
        ...
        <div id="full-preview-modal" class="fixed inset-0 flex flex-col items-center justify-center p-6">
            ...
            <div class="mb-10">
                <button id="download-btn-dynamic" class="download-badge px-8 py-4 rounded-2xl text-white font-black text-xs uppercase tracking-widest flex items-center gap-3">
                    Download
                </button>
            </div>
...
window.openFullPreview = (url) => {
    ...
    dlBtn.onclick = () => socialUtils.downloadImage(url);
    if(window.lucide) lucide.createIcons();
};
...
function renderPosts(posts, myPhone, isFirstLoad = false) {
    ...
    posts.forEach(post => {
        ...
        <div class="post-card p-2 animate-in fade-in duration-500">
            ...
            ${ytEmbedUrl ? `
                <div class="px-4 pb-4">
                    <div class="rounded-[30px] overflow-hidden aspect-video bg-black shadow-inner">
