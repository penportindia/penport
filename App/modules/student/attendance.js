...
window.attCache = window.attCache || {};

async function getMonthData(monthKey, sClass, sSec, sFolio) {
    const cacheKey = `${monthKey}_${sClass}_${sSec}_${sFolio}`;
    if (window.attCache[cacheKey]) {
        return window.attCache[cacheKey];
    }

    const snapshot = await db
        .ref(`attendance/${monthKey}/student/${sClass}/${sSec}/${sFolio}`)
        .once('value');

    const data = snapshot.val() || {};
    window.attCache[cacheKey] = data;
    return data;
}
...
window.render_att = async function(user) {
    ...
    try {
        ...
        const monthData = await getMonthData(monthKey, sClass, sSec, sFolio);
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            let status = monthData[dateStr] || null;
            const isSunday = new Date(year, month, day).getDay() === 0;
            
            let style = "bg-slate-50 text-slate-400 border-transparent";
            let displayValue = `<span class="text-sm font-bold">${day}</span>`;
            ...
            if (status === 'P') {
                ...
                displayValue = `<span class="text-[10px] font-black">${day}</span>`;
            }
        ...
        container.innerHTML = `
            <div class="mt-10 px-6 pb-12 max-w-md mx-auto">
