window.render_homework = async function (userData) {
    ...
    window.viewAttachments = function (hw) {
        let html = `<div class="grid grid-cols-1 gap-3 p-1" style="max-height:75vh; overflow-y:auto;">`;
        if (hw.images) {
            hw.images.forEach(url => {
                const isPDF = url.toLowerCase().includes('.pdf');
                html += `
                <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                    <div class="p-3 border-b border-slate-50 flex items-center gap-3 ${isPDF ? 'bg-rose-50/50' : 'bg-blue-50/50'} text-left">
                        ...
                        <div class="text-[11px] font-bold text-slate-800 uppercase">${isPDF ? 'PDF Document' : 'Image Preview'}</div>
                    </div>
                    ${isPDF ? 
                        `<iframe src="https://docs.google.com/viewer?url=${encodeURIComponent(url)}&embedded=true" class="w-full h-[400px] border-none"></iframe>` : 
                        `<div class="p-2"><img src="${url}" class="w-full h-auto rounded-xl" loading="lazy"></div>`
                    }
                </div>`;
            });
        }
        if (hw.videos) {
            hw.videos.forEach(v => {
                html += `
                <div onclick="window.open('${v}', '_blank')" class="bg-indigo-600 rounded-2xl p-4 flex items-center gap-4 shadow-lg cursor-pointer">
                    <div class="w-10 h-10 bg-indigo-500/50 rounded-xl flex items-center justify-center border border-indigo-400"><i data-lucide="play-circle" class="text-white w-6 h-6"></i></div>
                    <div class="flex-grow overflow-hidden text-left"><div class="text-[10px] text-indigo-100 font-bold uppercase tracking-widest">Video Lesson</div><div class="text-[11px] font-medium text-white truncate">${v}</div></div>
                </div>`;
            });
        }
        html += `</div>`;
        Swal.fire({
            html: html, showConfirmButton: false, showCloseButton: true, background: '#ffffff', width: '90%', position: 'bottom',
            customClass: { popup: 'rounded-[32px] border-none shadow-2xl animate__animated animate__slideInUp !mb-10' },
