window.homeworkCurrentUploadXHRs = window.homeworkCurrentUploadXHRs || {};
window.homeworkUploadedImageUrls = window.homeworkUploadedImageUrls || [];
window.allBranches = window.allBranches || [];
window.activeBranchName = window.activeBranchName || '';
window.homeworkListener = window.homeworkListener || null;
window.feedListener = window.feedListener || null;
...
window.render_homework = async function (user) {
    ...
    const styleInject = `
        <style>
            ...
            .module-container { padding: 16px; padding-top: 40px; max-width: 800px; margin: 0 auto; animation: fadeIn 0.5s ease; }
            .admin-master-panel {
...
window.loadAdminFeed = function () {
    ...
    window.feedListener = db.ref(`homework/${window.activeBranchName}`).on('value', (snap) => {
        ...
        const html = sortedItems.map(hw => {
            ...
            return `
            <div class="hw-card bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex gap-2">
                        <span class="bg-indigo-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase">Class ${hw.className}</span>
                        <span class="bg-slate-900 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase">${hw.section}</span>
                    </div>
                    <div class="flex items-center gap-1 text-[10px] font-black text-amber-600 bg-amber-50 px-2.5 py-1 rounded-lg">
                        <i data-lucide="shield-check" class="w-3.5 h-3.5"></i> MASTER CONTROL
                    </div>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <div class="task-container w-full p-4 bg-slate-50 rounded-2xl overflow-auto">
                        <div class="task-content w-full text-slate-700 leading-relaxed break-words whitespace-pre-wrap overflow-wrap-anywhere">${(hw.task || '').trim()}</div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2 mb-5">
                    <div class="flex items-center gap-1.5 opacity-70">
                        ...
                        <span class="text-[10px] font-bold uppercase tracking-tighter text-slate-500">${hw.teacherName}</span>
                    </div>
                    ${hw.editedBy ? `
                        <div class="flex items-center gap-2 bg-emerald-50 text-emerald-700 p-2.5 rounded-xl border border-emerald-100">
                            <i data-lucide="refresh-ccw" class="w-3.5 h-3.5"></i>
                            <span class="text-[10px] font-bold uppercase tracking-tighter text-emerald-600">${hw.editedBy}</span>
                        </div>
                    ` : ''}
                </div>

                <div class="flex items-center justify-between pt-4 border-t border-slate-100">
                    <button onclick='viewAttachments(${JSON.stringify(hw)})' class="flex items-center gap-2 bg-slate-100 text-slate-700 px-4 py-2.5 rounded-xl font-bold text-xs hover:bg-slate-200 transition-colors">
                        <i data-lucide="files" class="w-4 h-4"></i>
                        <span>${mediaCount > 0 ? mediaCount + ' Assets' : 'No Media'}</span>
                    </button>
                    
                    <div class="flex gap-2">
                        <button onclick="editHw('${hw.id}')" class="w-10 h-10 flex items-center justify-center text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-100 active:scale-95 transition-all">
                            <i data-lucide="edit" class="w-5 h-5"></i>
                        </button>
                        <button onclick="deleteHw('${hw.id}')" class="w-10 h-10 flex items-center justify-center text-rose-600 bg-rose-50 rounded-xl hover:bg-rose-100 active:scale-95 transition-all">
