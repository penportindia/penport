window.render_notice = async function(user) {
    ...
    mainContent.innerHTML = `
        <div id="notice-list" class="space-y-6 pb-32 pt-10 px-4">
        ...
        if (filteredData.length === 0) {
            ...
            filteredData.forEach(n => {
                ...
                const attachmentsHTML = n.images?.map((url, index) => {
                    ...
                    return `
                        <div onclick="window.downloadFile('${url}', '${fileName}')" class="group relative w-16 h-16 rounded-2xl overflow-hidden border border-slate-100 bg-slate-50 cursor-pointer active:scale-95 transition-transform">
                            ${isPDF ? `
