...
window.loadTeacherFeed = function (phone) {
    ...
    db.ref(`homework/${window.activeBranchName}`).on('value', (snap) => {
        ...
        snap.forEach(child => {
            ...
            if (
                dbDate === selDate &&
                (App.state.user.role !== 'teacher' || hw.teacherPhone === phone)
            ) {
        ...
        items.sort((a, b) => b.timestamp - a.timestamp).forEach(hw => {
            ...
            <div class="group relative bg-white rounded-[32px] p-6 mb-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 transition-all duration-300 hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 active:scale-[0.98]">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="flex items-center gap-1">
                            <div class="flex items-center justify-center bg-indigo-600/10 border border-indigo-100 px-3 py-1.5 rounded-xl shadow-sm">
                                <span class="text-indigo-600 text-[10px] font-black uppercase tracking-tight">
                                    ${hw.className}
                                </span>
                            </div>

                            <div class="flex items-center justify-center bg-slate-900 px-3 py-1.5 rounded-xl shadow-lg shadow-slate-200">
                                <span class="text-white text-[10px] font-black tracking-widest uppercase">
                                    ${hw.section}
                                </span>
                            </div>
                        </div>
                ...
                <div class="flex items-center gap-3 pt-5 border-t border-dashed border-slate-200">
                    <button onclick='viewAttachments(${JSON.stringify(hw)})' 
...
window.viewAttachments = function (hw) {
            ...
            if (isPDF) {
                ...
                <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    <div class="p-3 bg-white border-t border-slate-50">
                        <button onclick="window.downloadFile('${url}', '${fileName}')" class="w-full py-3 bg-slate-900 active:bg-black text-white rounded-xl text-[12px] font-bold flex items-center justify-center gap-2 transition-all active:scale-95">
                ...
                <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    <div class="p-3 bg-white border-t border-slate-50">
                        <button onclick="downloadFile('${url}', '${fileName}')" class="w-full py-3 bg-slate-900 active:bg-black text-white rounded-xl text-[12px] font-bold flex items-center justify-center gap-2 transition-all active:scale-95">
    ...
    if (hw.videos && hw.videos.length > 0) {
        hw.videos.forEach(v => {
            html += `
            <div class="bg-indigo-600 rounded-2xl p-4 flex items-center gap-4 shadow-lg shadow-indigo-100 active:scale-95 transition-transform" onclick="window.open('${v}', '_blank')">
...
async function saveHw(data, editId, originalInfo = null) {
    ...
    try {
        if (editId) await db.ref(`homework/${window.activeBranchName}/${editId}`).update(payload);
        else await db.ref(`homework/${window.activeBranchName}`).push(payload);
        Swal.fire({ icon: 'success', title: 'Published Successfully', showConfirmButton: false, timer: 1000 });
    } catch (e) { Swal.fire('Error', e.message, 'error'); }
}

window.editHw = (id) => {
    db.ref(`homework/${window.activeBranchName}/${id}`).once('value', s => {
        if (s.exists()) {
            const d = s.val();
...
window.deleteHw = (id) => {
    Swal.fire({ title: 'Delete?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444' })
        .then(r => { if (r.isConfirmed) db.ref(`homework/${window.activeBranchName}/${id}`).remove(); });
};
    ...
    const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
            const data = {
                ...
                fullDate: editId 
                    ? existingData.fullDate
                    : (document.getElementById('hw-filter-date')?.value || new Date().toISOString().split('T')[0])
            };
...
window.handleImageUpload = function (input) {
    ...
    Array.from(input.files).forEach(file => {
        ...
        xhr.onload = () => {
            ...
            if (Object.keys(window.currentUploadXHRs).length === 0) togglePublishBtn(true);
        };
...
window.removeImageFromQueue = (url, id) => { window.uploadedImageUrls = window.uploadedImageUrls.filter(u => u !== url); document.getElementById(id)?.remove(); };
