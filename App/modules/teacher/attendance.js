    ...
    <div class="max-w-md mx-auto space-y-4 pb-44 px-3 mt-6 animate-fade">
        ...
        <div id="section-mark" class="space-y-4">
            ...
            <div id="mark-controls" class="hidden space-y-3">
                <input type="text" id="att-search" onkeyup="filterStudents()" placeholder=" Search " 
        ...
        <div id="section-report" class="hidden space-y-4">
            <div class="glass-card p-5 space-y-4 bg-white/90 rounded-[2rem] shadow-xl border border-white/40">
                <div>
                    <label class="block text-[10px] font-black text-slate-400 mb-1.5 ml-1 uppercase">Target Date</label>
...
async function initAttLogic(branchName) {
    ...
    window.switchTab = (tab) => {
        ...
        document.getElementById('tab-mark').className = isMark ? 'flex-1 py-3 rounded-xl font-black text-[11px] transition-all bg-white text-blue-600 shadow-sm' : 'flex-1 py-3 rounded-xl font-black text-[11px] transition-all text-slate-500';
        document.getElementById('tab-report').className = !isMark ? 'flex-1 py-3 rounded-xl font-black text-[11px] transition-all bg-white text-blue-600 shadow-sm' : 'flex-1 py-3 rounded-xl font-black text-[11px] transition-all text-slate-500';
    };
}
...
window.fetchStudentList = async function(branchName) {
    ...
    if(!cls || !sec) return Swal.fire('Error', 'Class & Section select karein', 'warning');
...
// REPORT LOGIC WITH SORTING & FATHER NAME
window.generateReport = async function() {
    ...
    if(!cls || !sec) return Swal.fire('Error', 'Class/Section chunein', 'info');
    ...
    try {
        ...
        if(!records) {
            ...
            return content.innerHTML = `<div class="p-10 text-center text-rose-500 font-bold uppercase bg-rose-50 rounded-3xl">No Record Found</div>`;
        }

        // Prepare Students Data for Report
        let reportData = [];
        let p=0, a=0;
        ...
        stdSnap.forEach(s => {
            const data = s.val();
            const folio = s.key;
            if(data.academic?.class === cls && data.academic?.section === sec && records[folio]) {
                const status = records[folio];
...
window.saveAttendance = async function(cls, sec, date) {
    ...
    rows.forEach(r => {
        ...
        if(s) { records[r.getAttribute('data-folio')] = s; count++; }
    });
    if(count < rows.length) return Swal.fire('Error', 'Sab students ki hazri lagayein!', 'warning');
    try {
        Swal.fire({ title: 'Saving...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        await db.ref(`attendance/${month}/${date}/student/${cls}/${sec}`).set(records);
        await Swal.fire('Saved!', 'Hazri record ho gayi hai.', 'success');
        App.navigate('home');
