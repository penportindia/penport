...
async function render_exam() {
    ...
    const branch = App.state.branchName;
    ...
    content.innerHTML = `
    <div class="p-4 max-w-md mx-auto space-y-4 pt-10">
        <style>
        ...
        <div class="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-50 space-y-4">
            <div class="space-y-4">
                <div>
                    ...
                    <select id="f-session" onchange="currentFilters.session = this.value; loadExamTypes();" 
                ...
                <div>
                    ...
                    <select id="f-exam" onchange="currentFilters.examId = this.value; loadClasses();" 
                        class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-500 appearance-none">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select id="f-class" onchange="currentFilters.class = this.value; loadSections();" 
                            class="w-full bg-slate-50 border-none rounded-2xl p-4 font-bold text-slate-700 focus:ring-2 focus:ring-indigo-500 appearance-none">
        ...
        <div id="subject-grid" class="space-y-3"></div>
        <div id="marks-entry-container" class="hidden animate-in fade-in duration-300"></div>
    </div>
    `;
...
async function loadSections() {
    ...
    let sections = [];
    snap.forEach(c => {
        if(c.val().branch === branch && c.val().className === currentFilters.class) {
...
async function refreshSubjectList() {
    ...
    if(!currentFilters.section) return;

    const snap = await db.ref("exam_timetables").once("value");
    let html = "";
    snap.forEach(child => {
        const e = child.val();
        if(e.class === currentFilters.class && e.sessionId === currentFilters.session && e.examId === currentFilters.examId && e.section.includes(currentFilters.section)) {
            html += `
            <div onclick="openMarksEntry('${child.key}')" 
                class="bg-white p-5 rounded-[1.8rem] border border-slate-100 shadow-sm flex justify-between items-center active:scale-95 transition-transform">
                <div>
                    <h4 class="font-black text-slate-800">${e.subject}</h4>
    ...
    grid.innerHTML = html || `<p class="text-center text-slate-400 py-10">No schedule found.</p>`;
}
...
async function openMarksEntry(scheduleId) {
    ...
    container.innerHTML = `<div class="p-10 text-center text-slate-400 font-bold">Loading...</div>`;
    ...
    studentSnap.forEach(child => {
        ...
        if (s.academic.class === schedule.class && 
            ...
            s.academic.branch === App.state.branchName) {
    ...
    <div class="max-w-6xl mx-auto p-4">
        <div class="flex items-center gap-5 mb-8">
            <button onclick="document.getElementById('marks-entry-container').classList.add('hidden'); document.getElementById('subject-grid').classList.remove('hidden');" 
                class="h-12 w-12 bg-white shadow-sm border border-slate-200 rounded-2xl flex items-center justify-center text-slate-600">
                <i class="fa-solid fa-arrow-left"></i>
            </button>
            <div>
                <h3 class="font-black text-3xl text-slate-800">${schedule.subject}</h3>
                <p class="text-xs font-black text-indigo-500 uppercase tracking-widest">${schedule.examName} • SEC ${currentSec}</p>
            </div>
        </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            <th class="p-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-wider">Student Details</th>
                            ${Object.keys(schedule.components).map(comp => `
                                <th class="p-6 text-center text-[10px] font-black text-slate-400 uppercase tracking-wider">${comp}</th>
                            `).join('')}
                        </tr>
                    </thead>
                ...
                <td class="p-5">
                    <div class="flex items-center gap-4">
                        <div class="flex flex-col items-center">
                            ...
                            <span class="text-[9px] font-black text-slate-400 mt-1 uppercase">${s.profile.folio}</span>
                        </div>
                        <div>
            ...
            if (c.type === "number") {
                studentHtml += `
                    <td class="p-4 text-center">
                        <input type="number" value="${val}" id="mark-${id}-${comp}" placeholder="-"
                            onwheel="return false;"
                            class="w-32 mx-auto block bg-slate-50 border-2 border-transparent rounded-2xl p-4 text-center font-black text-slate-800 focus:bg-white focus:border-indigo-500 outline-none text-xl transition-all shadow-sm">
                    </td>`;
            } else {
                studentHtml += `
                    <td class="p-4 text-center">
                        <select id="mark-${id}-${comp}" 
                            class="w-36 mx-auto block bg-slate-50 border-2 border-transparent rounded-2xl p-4 font-bold text-slate-800 focus:bg-white focus:border-indigo-500 outline-none transition-all cursor-pointer">
                            <option value="">-</option>
    ...
    studentHtml += `
            ...
            <div class="p-8 bg-slate-50/50">
                <button onclick="saveExamMarks('${scheduleId}')" 
                    class="w-full bg-slate-900 hover:bg-black text-white py-6 rounded-[2rem] font-black text-xl shadow-xl active:scale-[0.98] transition-all tracking-wide">
                    SAVE
                </button>
            </div>
        </div>
...
async function saveExamMarks(scheduleId) {
    ...
    studentSnap.forEach(child => {
        const s = child.val();
        if (s.academic.class !== schedule.class || s.academic.section !== sec || s.academic.branch !== App.state.branchName) return;
