window.render_staff_list = async function(user) {
    ...
    mainContent.innerHTML = `
        <div class="min-h-screen bg-[#FDFDFD] pb-32 pt-10 px-6">
            <div class="mb-6 mt-2">
                <div class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <i data-lucide="search" class="w-5 h-5 text-slate-400"></i>
                    </div>
                    <input type="text" id="staff-global-input" placeholder=" Search " 
            ...
            <div class="mb-8">
                <div class="bg-white border border-slate-100 shadow-sm p-3 rounded-[24px] relative">
                    <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 block ml-2">Filter Designation</label>
                    <div class="relative">
            ...
            <div class="flex items-center justify-between px-2 mb-4">
                <h3 id="list-title" class="text-[10px] font-black text-slate-400 uppercase tracking-[2px]">Branch Staff</h3>
    ...
    const initData = async () => {
        ...
        try {
            ...
            const allStaffArr = Object.values(staffData);

            localStaffData = allStaffArr.filter(emp => emp.branch === userBranchId || userBranchId === "All");

            const designations = [...new Set(localStaffData.map(e => e.designation).filter(Boolean))];
            desSelect.innerHTML = '<option value="">All Staff</option>' + 
    ...
    window.handleGlobalStaffSearch = async () => {
        ...
        if (!query) {
            document.getElementById('list-title').innerText = "Branch Staff";
            renderStaffCards(localStaffData);
            return;
        }
        ...
        document.getElementById('list-title').innerText = "Global Search Results";

        try {
            const snapshot = await db.ref('employees').once('value');
            const data = snapshot.val() || {};
            const results = Object.values(data).filter(s => 
    ...
    window.applyLocalFilter = () => {
        ...
        const filtered = selDes === "" ? localStaffData : localStaffData.filter(e => e.designation === selDes);
        document.getElementById('list-title').innerText = "Branch Staff";
    ...
    const renderStaffCards = (list) => {
        ...
        container.innerHTML = list.map(s => {
            ...
            const isLocal = s.branch === userBranchId;
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="flex gap-1">
                                    <span class="text-[8px] font-black bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md uppercase">${s.empId}</span>
                    ...
                    <div class="bg-slate-50 rounded-2xl p-3 space-y-3">
                        <div class="flex justify-between items-center border-b border-slate-200/50 pb-2">
                            <div class="flex-1">
                                ...
                                <p class="text-[10px] font-black ${isLocal ? 'text-indigo-600' : 'text-orange-600'} uppercase flex items-center gap-1">
