window.render_att_take = async function(user) {
    ...
    container.innerHTML = `
    <div class="max-w-md mx-auto space-y-4 pb-44 px-3 mt-10 animate-fade">
...
window.saveAll = async function(path, date) {
    ...
    try {
        Swal.fire({ title: 'Marking...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        ...
        rows.forEach(r => {
            ...
            updates[`${path}/${id}/${date}`] = status;
        });

        await db.ref().update(updates);

        await Swal.fire('Success', 'Attendance Save', 'success');
        App.navigate('home');
    } catch(e) {
        Swal.fire('Wait!', e.message, 'warning');
    }
};
