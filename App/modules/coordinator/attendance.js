window.render_att_take = async function(user) {
    ...
    // Clean UI: No extra headers, purely functional
    container.innerHTML = `
    <div class="max-w-md mx-auto space-y-4 pb-44 px-3 mt-4 animate-fade">
        ...
        <div id="filter-card" class="glass-card p-5 grid grid-cols-2 gap-3 border border-white/40 shadow-2xl rounded-[2rem] bg-white/90">
            ...
            <div class="col-span-2">
                <button onclick="fetchAction('${myBranchId}', '${myUserId}')" class="w-full bg-slate-900 text-white p-4 rounded-2xl font-black text-xs shadow-xl uppercase active:scale-95 transition-all tracking-widest">
                    VIEW
                </button>
            </div>
        </div>
        ...
        <div id="section-mark" class="space-y-4">
            <div id="mark-controls" class="hidden space-y-3">
                ...
                <div class="flex gap-2">
                    <button onclick="markAllP()" class="flex-[2] bg-emerald-500 text-white py-4 rounded-2xl font-black text-[10px] shadow-lg uppercase tracking-tighter">
                        Mark All Present
                    </button>
                    <button id="quick-save-btn" onclick="triggerSave()" class="flex-1 bg-blue-600 text-white py-4 rounded-2xl font-black text-[10px] shadow-lg uppercase tracking-tighter">
                        Save
                    </button>
                </div>
            </div>
    ...
    // Initialize Branch & Class data
    const bSnap = await db.ref(`foundation/branches/${myBranchId}/name`).once('value');
    window.currentBranchName = bSnap.val() || "Main Branch";
    populateClasses(window.currentBranchName);
};
...
window.fetchAction = async function(bId, uId) {
    ...
    try {
        ...
        if(mode === 'student') {
            ...
            const stdSnap = await db.ref('student').orderByChild('academic/branch').equalTo(window.currentBranchName).once('value');
            ...
            stdSnap.forEach(s => {
                ...
                if(d.academic?.class === cls && d.academic?.section === sec) {
                    members.push({ 
                        id: s.key, 
                        folio: s.key,
                        name: d.profile?.studentName, 
                        roll: parseInt(d.academic?.rollNo) || 999,
                        father: d.parents?.father?.name || '---',
                        info: `F: ${d.parents?.father?.name || '---'}`, 
                        sub: `Folio: ${s.key}` 
                    });
                }
            });
            ...
            empSnap.forEach(e => {
                ...
                if(d.status === 'active' && e.key !== uId && !desig.includes('coordinator')) {
                    members.push({ 
                        id: e.key, 
                        name: d.name, 
                        roll: parseInt(d.empId) || 0, 
                        sub: d.designation || 'Staff', 
                        info: `Emp ID: ${d.empId || 'N/A'} | ${d.phone || '---'}` 
                    });
                }
            });
            members.sort((a, b) => a.roll - b.roll);
        ...
        else renderMarkView(members, savedData);
        ...
        <div class="att-row p-4 rounded-[1.8rem] bg-white border border-slate-100 flex items-center justify-between shadow-sm animate-fade-up" 
            ...
            <div class="min-w-0 flex-1 pr-3">
                <div class="flex items-center gap-2 mb-0.5">
                    ...
                    <p class="font-black text-slate-800 text-[13px] uppercase truncate">${m.name}</p>
                </div>
...
window.triggerSave = () => {
    if(!window.currentAttPath) return;
    saveAll(window.currentAttPath);
};
...
window.saveAll = async function(path) {
    ...
    rows.forEach(r => { 
        ...
        if(status) { data[r.getAttribute('data-id')] = status; count++; }
    });

    if(count < rows.length) return Swal.fire('Wait!', `Please mark all ${rows.length} members.`, 'warning');

    try {
        Swal.fire({ title: 'Saving...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        await db.ref(path).set(data);
        await Swal.fire('Success', 'Attendance saved for today!', 'success');
        App.navigate('home');
...
window.switchTab = (tab) => {
    ...
    // Yahan change kiya hai: Dono tabs mein col-span-1 rahega taaki side-by-side dikhein
    document.getElementById('wrapper-class').classList.add('col-span-1');
    document.getElementById('wrapper-class').classList.remove('col-span-2');
