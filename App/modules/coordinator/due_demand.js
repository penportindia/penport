(function () {
    ...
    const render = async () => {
        moduleContainer.innerHTML = `
            <div class="bg-slate-50 min-h-screen pb-28 pt-10">
                <div class="px-4">
    ...
    const setupFoundation = async () => {
        ...
        if (!branchSel || !classSel || !secSel || !monthSel) return;
        ...
        const sessionRaw = localStorage.getItem('school_app_session');
        ...
        const userBranch = session?.branchName;
        if (!userBranch) return;

        const branches = Object.values(FOUNDATION.branches || {});
        const branchExists = branches.some(b => b.name === userBranch);
        if (!branchExists) return;

        branchSel.add(new Option(userBranch, userBranch));
        ...
        const populateClasses = () => {
            ...
            const classes = Object.values(FOUNDATION.classes || {})
                .filter(c => c.branch === userBranch && c.className);

            const uniqueClasses = [...new Set(classes.map(c => c.className))];
            uniqueClasses.forEach(c => classSel.add(new Option(c, c)));
        };
        ...
        const populateSections = () => {
            ...
            const classData = Object.values(FOUNDATION.classes || {})
                .find(c => c.branch === userBranch && c.className === classSel.value);
        ...
        branchSel.addEventListener('change', populateClasses);
