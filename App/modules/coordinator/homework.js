window.currentUploadXHRs = window.currentUploadXHRs || {};
window.uploadedImageUrls = window.uploadedImageUrls || [];
window.activeBranchName = window.activeBranchName || "";
window.allBranches = window.allBranches || [];

window.render_homework = async function (user) {

    if (window.hwRef) window.hwRef.off();
    ...
    mainContent.innerHTML = `
        <style>
            :root { 
                ...
                --erp-primary:#4f46e5;
                --erp-dark:#1e293b;
                --erp-card-shadow:0 10px 25px rgba(0,0,0,0.05);
            }

            .module-container{padding:16px;padding-top:40px;max-width:800px;margin:auto}
            .date-strip{
            ...
            .enabled{background:#1e293b;color:#fff;cursor:pointer}
            .disabled{background:#e2e8f0;color:#94a3b8;pointer-events:none}
        ...
        <div class="module-container">
            ...
            <div class="date-strip flex items-center justify-between bg-white p-3 rounded-2xl shadow-sm">

                <div class="flex items-center gap-3">
                    <div class="p-3 bg-indigo-50 rounded-2xl flex items-center justify-center">
                        <i data-lucide="calendar" class="w-5 h-5 text-indigo-600"></i>
                    </div>

                    <input type="date" id="hw-filter-date" class="date-input" value="${today}">
                </div>

                <button id="coord-add-btn"
                    class="add-btn enabled w-11 h-11 rounded-xl flex items-center justify-center bg-slate-900 text-white active:scale-95 transition-all">
            ...
            <div class="filter-row">
                <select id="f-class" class="hw-select">
                    <option value="ALL">All Classes</option>
                </select>
                <select id="f-section" class="hw-select">
    ...
    const dateInput = document.getElementById('hw-filter-date');
    const addBtn = document.getElementById('coord-add-btn');

    if (dateInput) {
        dateInput.addEventListener('change', window.checkDateAndLoad);
    }

    if (addBtn) {
        addBtn.addEventListener('click', () => window.openHomeworkModal());
    }
...
window.loadCoordinatorFeed = function () {
    ...
    const refPath = `homework/${activeBranchName}`;

    if (window.hwRef) {
        window.hwRef.off();
    }

    window.hwRef = db.ref(refPath);

    window.hwRef.on('value', (snap) => {
        if (!snap.exists()) {
            listDiv.innerHTML = `
        ...
        let html = items.sort((a, b) => b.timestamp - a.timestamp).map(hw => {
            ...
            <div class="bg-white rounded-[28px] p-5 mb-4 shadow-sm border border-slate-100 relative overflow-hidden active:scale-[0.98] transition-transform">
                ...
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div class="flex items-center gap-1 text-[9px] font-black text-emerald-500 bg-emerald-50 px-2 py-1 rounded-lg">
                        <i data-lucide="shield-check" class="w-3 h-3"></i> COORDINATOR ACCESS
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <p class="w-full text-slate-600 text-[13px] leading-relaxed font-medium bg-slate-50/50 p-3 rounded-2xl border border-slate-50 whitespace-pre-wrap break-words overflow-wrap-anywhere">${hw.task.trim()}</p>
                </div>

                <div class="flex flex-col gap-1 mb-4">
                     <div class="flex items-center gap-1.5 opacity-60">
                        <i data-lucide="user" class="w-3 h-3"></i>
                        <span class="text-[9px] font-bold uppercase tracking-tighter text-slate-500">${hw.teacherName}</span>
                    </div>
                    ${hw.editedBy ? `
                        <div class="flex items-center gap-2 bg-emerald-50 text-emerald-700 p-2 rounded-xl border border-emerald-100">
                            ...
                            <span class="text-[9px] font-bold uppercase tracking-tighter text-emerald-600">${hw.editedBy}</span>
                        </div>
                    ` : ''}
                </div>
...
async function saveHw(data, editId, originalTeacher = null) {
    ...
    try {
        if (!window.activeBranchName) {
            Swal.fire('Error', 'Branch not set properly', 'error');
            return;
        }
