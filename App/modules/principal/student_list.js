window.render_student_list = async function(user) {
    ...
    // Config fetch kar rahe hain saare classes/sections nikalne ke liye
    const configSnap = await db.ref('foundation').once('value');
    const config = configSnap.val() || {};
    
    // Pure school ki sabhi classes (without branch filter)
    const allClassConfigs = Object.values(config.classes || {});
    
    // Unique class names nikalna taaki dropdown saaf rahe
    const uniqueClassNames = [...new Set(allClassConfigs.map(c => c.className))];
                ...
                <div class="bg-white border border-slate-100 shadow-sm p-3 rounded-[24px] relative">
                    ...
                    <div class="relative">
                        <select id="filter-class" onchange="updateSectionDropdown()" 
                            ...
                            ${uniqueClassNames.map(className => `<option value="${className}">${className}</option>`).join('')}
                        </select>
    ...
    window.updateSectionDropdown = () => {
        ...
        // Is class ke sabhi unique sections (across all branches)
        const classSections = new Set();
        allClassConfigs.forEach(c => {
            if(c.className === selClass && c.sections) {
                c.sections.split(',').forEach(s => classSections.add(s.trim()));
            }
        });
    ...
    window.fetchStudentsByQuery = async () => {
        ...
        try {
            ...
            // Yahan se branch filter hata diya gaya hai taaki pure school ka data aaye
            let filtered = Object.values(data).filter(s => 
                s.academic?.section === selSec
            );
    ...
    // handleFolioSearch ditto waisa hi hai
                    ...
                    <div class="text-left mt-2">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs font-medium text-slate-600 leading-relaxed">${s.contact?.address || 'N/A'}</p>
                            </div>
