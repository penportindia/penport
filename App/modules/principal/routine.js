async function render_routine(user) {
    ...
    try {
        ...
        if (!window.selectedBranch) {
            window.selectedBranch = user.branch || Object.keys(branches)[0];
        }

        mainContent.innerHTML = `
            <div class="min-h-screen bg-[#FAFBFF] pt-5 pb-20">
                <div class="sticky top-0 bg-[#FAFBFF]/95 backdrop-blur-sm z-50 border-b border-slate-200/50 shadow-sm">
                    <div class="px-5 pt-6 pb-2">
                        <div class="flex items-center gap-3 mb-4">
                            <div class="flex-1">
                                <select onchange="window.changeBranch(this.value)" 
                                    class="w-full appearance-none bg-slate-100/50 border border-slate-200/60 rounded-xl py-2.5 px-4 text-[12px] font-bold text-indigo-600 uppercase tracking-wide outline-none cursor-pointer text-center transition-all focus:bg-white focus:ring-2 focus:ring-indigo-500/10">
                                    ${Object.keys(branches).map(bk => `
                                        <option value="${bk}" ${window.selectedBranch === bk ? 'selected' : ''}>${branches[bk].name} Branch</option>
                                    `).join('')}
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="px-5 mb-4">
                        <div class="grid grid-cols-3 md:grid-cols-6 gap-2 bg-slate-200/50 p-1.5 rounded-[22px]">
                            ${['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map(day => {
                                const isToday = (window.selectedDay || new Date().toLocaleDateString('en-US', {weekday: 'long'})) === day;
                                return `
                                <button 
                                    onclick="setDay('${day}')" 
                                    data-day="${day}" 
                                    class="day-pill py-2.5 rounded-[18px] text-[11px] font-bold transition-all duration-300
                                    ${isToday ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}">
                ...
                <div id="analyzer-output" class="px-5 py-6 space-y-4"></div>
            </div>`;

        window.changeBranch = (val) => {
            window.selectedBranch = val;
        ...
        window.setDay = (day) => {
            document.querySelectorAll('.day-pill').forEach(btn => {
                btn.className = 'day-pill py-2.5 rounded-[18px] text-[11px] font-bold transition-all duration-300 text-slate-500 hover:text-slate-700';
            });
            const activeBtn = document.querySelector(`[data-day="${day}"]`);
            if(activeBtn) activeBtn.className = 'day-pill py-2.5 rounded-[18px] text-[11px] font-bold transition-all duration-300 bg-white text-indigo-600 shadow-sm';
        ...
        window.updateAnalysis = () => {
            ...
            const activeBranchName = branches[window.selectedBranch]?.name || window.selectedBranch;
        ...
        window.switchView = (v) => {
            ...
            document.getElementById('tab-section').className = v === 'section' ? 'flex-1 py-2.5 rounded-xl font-bold text-[11px] transition-all bg-white shadow-sm text-indigo-600 uppercase' : 'flex-1 py-2.5 rounded-xl font-bold text-[11px] transition-all text-slate-500 uppercase';
            document.getElementById('tab-teacher').className = v === 'teacher' ? 'flex-1 py-2.5 rounded-xl font-bold text-[11px] transition-all bg-white shadow-sm text-indigo-600 uppercase' : 'flex-1 py-2.5 rounded-xl font-bold text-[11px] transition-all text-slate-500 uppercase';
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="flex items-center gap-2 mt-0.5">
                                    <span class="text-[10px] text-indigo-500 font-bold decoration-indigo-100 underline-offset-2 underline">${bookName}</span>
