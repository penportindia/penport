...
window.applyFilters = function() {
    ...
    let filtered = allNotices.filter(n => 
        ...
        (n.msg && n.msg.toLowerCase().includes(searchTerm))
    );
...
function renderFeed(data) {
    ...
    if(!feed) return;
    if(!data.length) { feed.innerHTML = `<div style="text-align:center; padding:40px;">No records found</div>`; return; }
            ...
            <div class="notice-card ${n.pinned ? 'is-pinned' : ''}">
                ...
                <div class="card-footer">
                    ...
                    <div class="action-btns">
                        <button onclick="togglePin('${n.id}', ${n.pinned})" class="btn-circle btn-pin ${n.pinned ? 'active' : ''}"><i data-lucide="pin" style="width:16px"></i></button>
                        <button onclick="deleteNotice('${n.id}')" class="btn-circle btn-del"><i data-lucide="trash-2" style="width:16px"></i></button>
                    </div>
                </div>
            </div>`;
    }).join('');
...
window.openNoticeModal = async function() {
    ...
    const { value: form } = await Swal.fire({
        ...
        background: '#ffffff',
        customClass: {
            popup: 'clean-mobile-modal',
            confirmButton: 'clean-confirm-btn'
        },
        html: `
            <style>
                ...
                .input-field { 
                    width: 100%; padding: 12px; border-radius: 8px; border: 1px solid #E5E5E5;
                    background: #F9F9F9; font-size: 15px; color: #333; box-sizing: border-box;
                    transition: all 0.2s ease; outline: none; -webkit-appearance: none;
                }
                ...
                .upload-box {
                    border: 1px dashed #CCC; background: #FAFAFA; padding: 15px;
                    border-radius: 10px; text-align: center; margin-top: 5px;
                }
                .upload-btn {
                    background: #FFF; border: 1px solid #DDD; padding: 8px 16px;
                    border-radius: 6px; font-size: 13px; color: #444; font-weight: 500;
                }
                #upload-status-box { margin-top: 10px; display: flex; gap: 5px; flex-wrap: wrap; }
                .preview-thumb { width: 45px; height: 45px; border-radius: 4px; object-fit: cover; border: 1px solid #EEE; }
            </style>
            ...
            <div class="notice-form-container">
                ...
                <div class="form-group">
                    ...
                    <textarea id="sw-msg" class="input-field" style="height:250px; resize:none;" placeholder="Write massage here..."></textarea>
                </div>
        ...
        preConfirm: () => {
            ...
            if(!data.title || !data.msg) return Swal.showValidationMessage('Required fields missing');
            if(Object.keys(currentUploadXHRs).length > 0) return Swal.showValidationMessage('Uploading...');
            return data;
        }
    });
    ...
    if(form) {
        ...
        Toast.fire({ icon: 'success', title: 'Notice Sent' });
    }
};
...
window.loadSectionsV4 = function(className) {
    ...
    const branch = document.getElementById('sw-branch').value;
    if(!secSel) return;
    if(className === 'ALL') { secSel.innerHTML = '<option value="ALL">All Sections</option>'; return; }
...
window.handleImageUpload = function(input) {
    ...
    Array.from(input.files).forEach(file => {
        ...
        xhr.onload = () => {
            ...
            if(res.secure_url) {
                tempNoticeImages.push(res.secure_url);

                document.getElementById(upId).innerHTML = `<span style="color:var(--success); font-size:11px; font-weight:700">✓ ${finalFileName}</span><button onclick="removeImgV4('${upId}', '${res.secure_url}')" style="margin-left:auto; border:none; background:none; color:red; font-weight:800">×</button>`;
            }
