window.render_att_take = async function(user) {
    ...
    // Clean UI: Minimal, Mobile-Friendly, Coordinator Style
    container.innerHTML = `
...
window.fetchAction = async function() {
    ...
    try {
        ...
        if(mode === 'student') {
            ...
            empSnap.forEach(e => {
                const d = e.val();
                const empBranch = (d.branch || "").toLowerCase();
                // Condition: Show if matches selected branch OR is 'all' OR is empty/undefined
                if(d.status === 'active' && (d.branch === bId || empBranch === 'all' || !d.branch)) {
...
window.saveAll = async function(path) {
    ...
    if(count < rows.length) return Swal.fire('Incomplete', `Please mark all ${rows.length} members.`, 'warning');

    try {
        Swal.fire({ title: 'Saving...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        await db.ref(path).set(data);
        await Swal.fire('Success', 'Attendance synchronized!', 'success');
        App.navigate('home');
