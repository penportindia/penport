...
window.render_att_view = async function(user) {
    ...
    const myUserId = String(user?.uid || user?.empId || "101"); 
    ...
    try {
        // Hum pure staff node ko fetch kar rahe hain taaki kisi bhi branch mein dhoondh sakein
        const dbPath = `attendance/${monthKey}/staff`;
        const attSnap = await db.ref(dbPath).once('value');
        const allBranchesData = attSnap.val() || {};
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            // SARI BRANCHES MEIN USER KO DHOONDHNA
            // Hum allBranchesData (jo ki har branch ka data hai) ke andar loop chala rahe hain
            for (const branchId in allBranchesData) {
                const branchStaff = allBranchesData[branchId];
                if (branchStaff[myUserId] && branchStaff[myUserId][dateStr]) {
                    status = branchStaff[myUserId][dateStr];
                    break; // Agar status mil gaya toh aage search karne ki zaroorat nahi
                }
            }
        ...
        container.innerHTML = `
            <div class="mt-6 px-6 pb-12 max-w-md mx-auto animate-fade-in">
                <div class="flex items-center justify-between mb-8 bg-white p-2 rounded-3xl shadow-sm border border-slate-100">
                    <button onclick="window.changeMonth(-1, '${encodeURIComponent(JSON.stringify(user))}')" class="p-3 bg-slate-50 rounded-2xl text-slate-600 active:bg-indigo-500 active:text-white transition-all">
                        <i data-lucide="chevron-left" class="w-5 h-5"></i>
                    </button>
                    <div class="text-center">
                        <h2 class="text-lg font-black text-slate-800 leading-none">${monthName}</h2>
                        <p class="text-[10px] font-bold text-indigo-500 uppercase tracking-tighter mt-1">${year} ATTENDANCE</p>
                    </div>
                    <button onclick="window.changeMonth(1, '${encodeURIComponent(JSON.stringify(user))}')" class="p-3 bg-slate-50 rounded-2xl text-slate-600 active:bg-indigo-500 active:text-white transition-all">
        ...
        container.innerHTML = `<div class="p-10 text-center text-slate-500 font-bold uppercase text-[10px]">Data load nahi ho saka</div>`;
    }
};
